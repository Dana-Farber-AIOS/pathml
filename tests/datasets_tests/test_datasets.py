...
from pathml.datasets.datasets import TileDataset, EntityDataset
...
def create_test_h5_file():
    ...
    with h5py.File(h5_path, "w") as f:
        ...
        for i in range(5):
            ...
            array_data = np.random.rand(224, 224, 3).astype('float32')  # Ensure data type matches expected torch.Tensor type
            tile.create_dataset("array", data=array_data)
            if i % 2 == 0:  # Add masks to some tiles
                masks = tile.create_group("masks")
                masks.create_dataset("mask1", data=np.random.randint(2, size=(224, 224)).astype('float32'))
...
def test_tile_dataset_getitem(create_test_h5_file):
    ...
    for i in range(len(dataset)):
        ...
        assert im.shape == (3, 224, 224), "Image tensor shape should match expected (C, H, W)"
        if masks is not None:
            assert masks.shape[0] > 0 and masks.shape[1:] == (224, 224), "Mask shape should be (n_masks, H, W)"
        assert "example_label" in lab_tile, "Tile labels should include 'example_label'"
        assert lab_slide["slide_label"] == "slide_value", "Slide label should match expected value"
...
def test_tile_dataset_unsupported_shape_explicit_check(create_test_h5_file):
    ...
    with h5py.File(h5_path, "r+") as f:
        ...
        del f["tiles"]["0"]["array"]
        f["tiles"]["0"].create_dataset("array", data=np.random.rand(10, 10))  # 2D array, unsupported
