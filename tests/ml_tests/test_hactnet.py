...
def fake_hactnet_inputs():
    ...
    data = HACTPairData(x_cell=cell_features, 
...
def test_hovernet_forward_pass(batch_size, readout_op):
    ...
    cell_params = {'layer':'PNAConv', 'in_channels':256, 'hidden_channels':64, 
                   'num_layers':3, 'out_channels':64, 'readout_op':readout_op, 
                   'readout_type':'mean', 'kwargs':kwargs_pna_cell}
    
    tissue_params =  {'layer':'PNAConv', 'in_channels':256, 'hidden_channels':64, 
                      'num_layers':3, 'out_channels':64, 'readout_op':readout_op, 
