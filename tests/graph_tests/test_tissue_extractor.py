...
def make_fake_image(instance_map):
    ...
    image[image > 0] = 1
    noised_image = np.random.rand(instance_map.shape[0], instance_map.shape[1], 3)*image*255
...
def test_feature_extractor(entity, patch_size, threshold):
    ...
    instance_map = make_fake_instance_maps(num=20, image_size=image_size, ellipse_height=20, ellipse_width=8)
    image =  make_fake_image(instance_map.copy())
    regions = regionprops(instance_map)
    
    extractor = DeepPatchFeatureExtractor(patch_size=patch_size, 
                                            batch_size=1, 
                                            entity = entity,
