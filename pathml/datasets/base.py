...
class SlideDataset(data.Dataset):
    """
    A default Dataset object for a collection of whole-slide-images.
    In this basic dataset, slide labels are inputted as lists.
    Each item is a tuple of (slide, label)

    Args:
        slide_type (str): Type of slide. Must be a supported slide type. Currently 'HE' and 'multiplex' are supported.
        paths (list): List of paths to images.
        labels (list): List of slide labels. Must be same length as input paths.
    """
...
class TileDataset(data.Dataset):
    """
    A default Dataset object for a collection of tiles.
    Supports both tile-level and slide-level labels.
    In this basic dataset, tile-level and slide-level labels are inputted as lists.
    Each item is a tuple of (tile image, tile label, slide label).

    Args:
        paths (list): List of paths to tile images.
        tile_labels (list): List of tile-level labels. Must be same length as input paths.
        slide_labels (list): List of slide-level labels. Must be same length as input paths.
...
class DataModule:
    """
    A Default DataModule for handling datasets and creating dataloaders.
    ...
    def __init__(self, slide_type, slide_anno, tile_anno, tile_transforms, batch_size, random_slide_split,
        ...
        assert "slide_label" in slide_anno.columns, \
            "Error: input slide_anno dataframe must contain 'slide_label' column"
