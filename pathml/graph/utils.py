...
class HACTPairData(Data):
    ...
    def __init__(self, x_cell, edge_index_cell, x_tissue, edge_index_tissue, assignment, target):
        super().__init__()
        self.x_cell = x_cell
...
def get_full_instance_map(wsi, patch_size, mask_name='cell'):
    ...
    for tile in wsi.tiles:
        ...
        instance_map[tx:tx+patch_size, ty:ty+patch_size] = tile.masks[mask_name][:,:,0] 
...
def build_assignment_matrix(low_level_centroids, high_level_map, matrix=False):
    ...
    low_to_high = high_level_map[low_level_centroids[:, 1],low_level_centroids[:, 0]].astype(int)
