...
class SlideType:
    ...
    Examples:
        ...
        True
    """
    def __init__(self, stain=None, platform=None, tma=None, rgb=None, volumetric=None, time_series=None):
        # verify inputs
        valid_stains = {'HE', 'IHC', 'Fluor'}
        if stain and stain not in valid_stains:
            raise ValueError(f"Input stain {stain} not valid. Must be one of {valid_stains}")
        valid_platforms = {'CODEX', 'Vectra', 'Visium'}
        if platform and platform not in valid_platforms:
            raise ValueError(f"Input stain {platform} not valid. Must be one of {valid_platforms}")
        ...
        if tma == 0: tma = None
        if rgb == 0: rgb = None
        if volumetric == 0: volumetric = None
        if time_series == 0: time_series = None
...
class _PremadeTypes:
    ...
    def __init__(self):
        self.HE = SlideType(stain = 'HE', platform = None, tma = False, rgb = True, volumetric = False, time_series = False)
        self.IHC = SlideType(stain = 'IHC', platform = None, tma = False, rgb = True, volumetric = False, time_series = False)
        self.IF = SlideType(stain = 'Fluor', platform = None, tma = False, rgb = False, volumetric = False, time_series = False)
        self.CODEX = SlideType(stain = 'Fluor', platform = "CODEX", tma = False, rgb = False, volumetric = False, time_series = False)
        self.Vectra = SlideType(stain = 'Fluor', platform = "Vectra", tma = False, rgb = False, volumetric = False, time_series = False)
